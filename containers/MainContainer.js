...
const MainContainer = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchLocationsHandler = async (url) => {
      ...
      const fetchedLocation = await response.data;
      updatedLocations.push(fetchedLocation);
      dispatch(
        episodesActions.saveLocations({
    ...
    try {
      ...
      for (let i = 1; i <= 108; i++) {
        fetchLocationsHandler("https://rickandmortyapi.com/api/location/" + i);
      }
    } catch (error) {
      console.log(error);
    }
